...
export function drawClock(persona, selectedHour, hourlyStats) {
  ...
  for (let r = R_INNER; r <= R_OUTER; r += (R_OUTER - R_INNER) / 4) {
    ...
    ring.setAttribute('stroke', '#1e1710')
  ...
  amLabel.setAttribute('fill', '#3d3028')
  ...
  pmLabel.setAttribute('fill', '#3d3028')
  ...
  for (let h = 0; h < 24; h++) {
    ...
    path.setAttribute('fill', sounds.length > 0 ? color : '#130e0a')
    ...
    path.setAttribute('stroke', '#09070a')
  ...
  centerCircle.setAttribute('fill', '#09070a')
  ...
  if (persona) {
    ...
    roleEl.setAttribute('fill', '#6b5f52')
    ...
    boroughEl.setAttribute('fill', '#3d3028')
